...
import StyledCollapsibleRow, { BannerArea, StyledRow } from "./leadstab.style";
...
const LeadsTab = ({ onChatClick, setLeadsData }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (response && Array.isArray(response.data.payload)) {
  ...
  const getCategoryStatus = (slot) => {
    if (slot.completed === true) return "Completed";
    if (slot.noshow === true) return "No Show";
    if (slot.cancelled === true) return "Cancelled";
    if (slot.rescheduled === true) return "Rescheduled";
    if (slot.booked === true && slot.confirmed !== true) return "Booked";
    if (slot.confirmed === true) return "Confirmed";
    return "Open";
  };
      ...
      <StyledCollapsibleRow>
        <div className="row-header" onClick={handleToggle}>
          ...
          <span className="date-time">
            ...
            <span className="time">{formatTime(slot.startTime)}</span>
          </span>
          <FontAwesomeIcon
